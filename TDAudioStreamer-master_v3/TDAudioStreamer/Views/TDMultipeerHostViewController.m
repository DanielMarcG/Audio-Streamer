...
//        self.albumImage.image = nil;
    ...
    if (peers.count) {
        ...
        // --* this section needs to be replaced with microphone, mapping the microphone audio unit to NSOutputStream
        ///////////////////////////////////////////////////////
        ///////////////////////////////////////////////////////
...
- (IBAction)talkSwitch:(id)sender
{
    if (!talkSwitch.on) {
        ...
        NSLog(@"talk switch off");
        
    } else {
        NSArray *peers = [self.session connectedPeers];
        NSLog(@"connected to %@", peers);
        if (peers.count) {

            // initializes the NSOutputStream, in this case naming it "music" (from OutputStreamForPeer)
            self.outputStreamer = [[TDAudioOutputStreamer alloc] initWithOutputStream:[self.session outputStreamForPeer:peers[0]]];
            
            // initialize microphone audio unit
            [self.outputStreamer initializeAudio];
            [self.outputStreamer startAudioUnit];
