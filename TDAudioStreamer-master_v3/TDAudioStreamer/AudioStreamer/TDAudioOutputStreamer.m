...
static OSStatus recordingCallback(void *inRefCon,
     ...
     */
    TDAudioOutputStreamer *audioProcessor = (__bridge TDAudioOutputStreamer*) inRefCon;
...
static OSStatus playbackCallback(void *inRefCon,
	...
	for (int i=0; i < ioData->mNumberBuffers; i++) {
        ...
        NSLog(@"copy buffer to audio buffer");
        
        //send buffer to Output Stream
        //NSLog(@"send buffer to output stream from playbackcallback");
        //[audioProcessor sendDataChunk:&buffer];
...
{
    // initialize the microphone audio units
    gain = 0;
...
- (void)sendDataChunk
{
        ...
        // this is where the data gets sent to the other device
          NSLog(@"send Data");   
...
- (void)audioStream:(TDAudioStream *)audioStream didRaiseEvent:(TDAudioStreamEvent)event
{
    switch (event) {
        case TDAudioStreamEventWantsData:
            ...
            NSLog(@"TDAudioStreamEventWantsData");
            //DataFlag = YES;
            [self sendDataChunk];
